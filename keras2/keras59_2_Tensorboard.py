...
reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=7, factor=0.5, verbose=1,mode = 'auto')#

td = TensorBoard(log_dir='D:/study_data/tensorboard_log/_graph', histogram_freq=0, write_graph=True, write_images=True)
...
import time 
start = time.time()
hist = model.fit(x_train, y_train, epochs=10, batch_size=128, verbose=1, validation_split=0.2, callbacks=[es, reduce_lr,td])
end = time.time() - start
...
# (tf282gpu) D:\study_data\tensorboard_log>cd _graph
