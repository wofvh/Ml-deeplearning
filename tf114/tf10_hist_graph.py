...
optimizer = tf.train.GradientDescentOptimizer(learning_rate=0.172) #경사하강법 최적화 함수 #최적화 함수를 이용해서 손실함수의 미분값을 최소화한다.
train = optimizer.minimize(loss) #최적화 함수를 이용해서 손실함수의 미분값을 최소화한다. #loss를 최소화하는 최적화 함수
#3-2. 훈련

loss_val_list = [] #손실함수의 값을 저장할 리스트
w_val_list = [] 
...
plt.xlabel('epochs',marker='.',c='red',label='loss')
plt.ylabel('loss',marker='.',c='blue',label='val_loss')
plt.grid()
plt.show()
plt.title('show')
...
plt.plot(w_val_list)
plt.xlabel('epochs',marker='.',c='red',label='loss')
plt.ylabel('weights',marker='.',c='blue',label='val_loss')
plt.title('show') 
plt.legend()
plt.show()
